...
mailCheck.refetch = function () {
  if (_.isUndefined(mailCheck.fetchMailOptions)) {
    logger.warn('Mailcheck.refetch() running before Mailcheck.init(); please run Mailcheck.init() prior')
    return
  }
          ...
          async.waterfall(
            [
              ...
              function (results, next) {
                if (_.size(results) < 1) {
                  logger.debug('MailCheck: Nothing to Fetch.')
                  return next()
                }

                logger.debug('Processing %s Mail', _.size(results))
  ...
  messages.forEach(function (message) {
    if (
      ...
      async.auto(
          ...
          handleCreateTicket: [
            ...
            function (results, callback) {
              ...
              Ticket.create(
                ...
                function (err, ticket) {
                  if (err) {
                    logger.warn('Failed to create ticket from email: ' + err)
                    return callback(err)
                  }
        ...
        function (err) {
          logger.debug('Created %s tickets from mail', count)
          if (err) logger.warn(err)
          return done(err)
        }
      )
