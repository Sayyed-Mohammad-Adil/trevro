...
const ES = {}
ES.indexName = process.env.ELASTICSEARCH_INDEX_NAME || 'trevyro'

const checkConnection = callback => {
  const errorText = 'Elasticsearch client not initialized. Restart Trevyro!'
  ...
  return new Promise((resolve, reject) => {
    ;(async () => {
      try {
        ...
        else ES.host = config.elasticsearch.host + ':' + config.elasticsearch.port
...
ES.setupHooks = () => {
  ...
  emitter.on('ticket:created', data => {
    ticketSchema.getTicketById(data.ticket._id, function (err, ticket) {
      if (err) {
        logger.warn('Elasticsearch Error: ' + err)
        return false
      }
      ...
      ES.esclient.index(
        ...
        function (err) {
          if (err) logger.warn('Elasticsearch Error: ' + err)
        }
      )
    })
  })
...
ES.rebuildIndex = async () => {
  ...
  try {
    ...
    esFork.on('exit', function () {
      logger.debug('Rebuilding Process Closed: ' + esFork.pid)
      global.esRebuilding = false
