...
function performBackup (dbVersion, callback) {
  const child = require('child_process').fork(path.join(__dirname, '../../src/backup/backup'), {
    env: {
      ...
      FILENAME: 'PREUPGRADE--trevyro-v' + dbVersion + '-' + moment().format('MMDDYYYY_HHmm') + '.zip'
    }
  })
...
function removeAgentsFromGroups (callback) {
  // logger.debug('Migrating Agents from Groups...')
...
function createTicketStatus (callback) {
  ...
  async.series(
    [
      ...
      function (next) {
        logger.info('Updating ticket statuses for migration. Please Wait...')
        logger.debug('Status [New ID]: ' + newId)
        logger.debug('Status [Open ID]: ' + openId)
        logger.debug('Status [Pending ID]: ' + pendingId)
        logger.debug('Status [Closed ID]: ' + closedId)
      ...
      function (next) {
        logger.info('Completed updating ticket status.')
...
migrations.run = function (callback) {
  ...
  async.series(
    [
      ...
      function (next) {
        console.log(semver.satisfies(semver.coerce(databaseVersion).version));
