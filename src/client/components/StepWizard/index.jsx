...
class StepWizard extends React.Component {
  ...
  componentDidMount () {
    ...
    this.props.socket.on('$trevyro:accounts:import:onStatusChange', this.onImportStatusChange)
  }

  componentWillUnmount () {
    this.props.socket.removeAllListeners('$trevyro:accounts:import:onStatusChange')
  }
    ...
    wizard.steps({
      ...
      onStepChanged: (event, currentIndex) => {
        if (currentIndex === 2) {
          ...
          setTimeout(() => {
            // Importing...
            this.props.socket.emit('$trevyro:accounts:importer:send_csv')
          }, 1000)
        }
