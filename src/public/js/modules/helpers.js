...
define([
  ...
  helpers.forceSessionUpdate = () => {
    return new Promise((resolve, reject) => {
      if (!window.trevyroSessionService) return reject(new Error('SessionService is not loaded'))

      window.trevyroSessionService.forceUpdate(() => {
        const refreshedSessionUser = window.trevyroSessionService.getUser()
  ...
  helpers.UI.bindExpand = function () {
    ...
    if (menuButton.length > 0) {
      ...
      menuButton.on('click', function (e) {
        ...
        if ($('.sidebar').hasClass('expand')) {
          Cookies.set('$trevyro:sidebar:expanded', true, { expires: 999 })
        } else {
          Cookies.set('$trevyro:sidebar:expanded', false, { expires: 999 })
        }
      })
    }
  }
  ...
  helpers.UI.initSidebar = function () {
    if (Cookies.get('$trevyro:sidebar:expanded') === 'true') {
  ...
  helpers.getTimezone = function () {
    const sessionUser = window.trevyroSessionService.getUser()
    ...
    const settings = window.trevyroSettingsService.getSettings()
    return settings.timezone.value || 'America/New_York' // Return America/New_York as the fallback
  }

  helpers.getTimeFormat = function () {
    if (window.trevyroSettingsService) {
      return window.trevyroSettingsService.getSettings().timeFormat.value
    }
  ...
  helpers.getShortDateFormat = function () {
    if (window.trevyroSettingsService) {
      return window.trevyroSettingsService.getSettings().shortDateFormat.value
    }
  ...
  helpers.getLongDateFormat = function () {
    if (window.trevyroSettingsService) {
      return window.trevyroSettingsService.getSettings().longDateFormat.value
    }
  ...
  function newMessageSubmit (e) {
    ...
    $.ajax({
      ...
      .error(function (err) {
        ...
        console.log('[trevyro:helpers:newMessageSubmit] Error - ' + err)
      })
  }

  helpers.canUser = function (a, adminOverride) {
    let role = window.trevyroSessionService.getUser().role
    const roles = window.trevyroSessionService.getRoles()
  ...
  helpers.parseRoleHierarchy = function (roleId) {
    var roleOrder = window.trevyroSessionService.getRoleOrder()
  ...
  helpers.getLoggedInRoleHierarchy = function () {
    var loggedInRole = window.trevyroSessionService.getUser().role
  ...
  helpers.getRolesByHierarchy = function () {
    ...
    var roles = window.trevyroSessionService.getRoles()
    var returnedRoles = []
  ...
  helpers.hasPermOverRole = function (ownerRole, extRole, action, adminOverride) {
    ...
    if (!extRole) extRole = window.trevyroSessionService.getUser().role
    ...
    if (adminOverride === true) {
      if (extRole && extRole.isAdmin) {
        ...
        const r = window.trevyroSessionService.getRoles()
  ...
  helpers.flushRoles = function () {
    window.trevyroSessionService.flushRoles()
