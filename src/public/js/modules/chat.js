...
define('modules/chat', ['jquery', 'underscore', 'moment', 'modules/helpers', 'uikit', 'autogrow', 'history'], function (
  ...
  chatClient.init = function (sock) {
    loggedInAccount = window.trevyroSessionService.getUser()
    socket = sock
    ...
    socket.removeAllListeners('$trevyro:chat:updateOnlineBubbles')
    socket.on('$trevyro:chat:updateOnlineBubbles', function (data) {
      var $u = _.throttle(
        function () {
    ...
    socket.on('spawnChatWindow', function (data) {
      ...
      if (messageContent.length > 0) {
        ...
        startConversation(loggedInAccountId, data._id, function (err, convo) {
          if (err) {
            console.log('[trevyro:chat:openChatWindow] - Error')
            console.error(err)
    ...
    socket.removeAllListeners('$trevyro:messages:ui:receive')
    socket.on('$trevyro:messages:ui:receive', function (data) {
      ...
      $.event.trigger('$trevyro:chat:message', data)

      // Ajaxify Any ticket links
      $('body').ajaxify()
    })

    socket.removeAllListeners('$trevyro:messages:ui:user_typing')
    socket.on('$trevyro:messages:ui:user_typing', function (data) {
      $.event.trigger('$trevyro:chat:typing', data)
    ...
    socket.on('chatStopTyping', function (data) {
      $.event.trigger('$trevyro:chat:stoptyping', data)
    })

    socket.removeAllListeners('leftChatServer')
    socket.on('leftChatServer', function (data) {
      $.event.trigger('$trevyro:chat:leftchatserver', data)
    })

    $(window).on('$trevyro:chat:stoptyping.chatSystem', function (event, data) {
      var chatBox = []
      var scroller
  ...
  chatClient.sendChatMessage = function (cid, toUserId, message, complete) {
    ...
    $.ajax({
      ...
      error: function (error) {
        console.log('[trevyro:chat:bindActions] Error')
        console.log(error)
  ...
  chatClient.setUserIdle = function () {
    socket.emit('$trevyro:setUserIdle')
  }

  chatClient.setUserActive = function () {
    socket.emit('$trevyro:setUserActive')
  }

  chatClient.updateOnlineBubbles = function () {
    socket.emit('$trevyro:chat:updateOnlineBubbles')
  }
