...
define('pages/messages', [
  ...
  messagesPage.init = function (callback) {
    $(document).ready(function () {
      ...
      const $loggedInAccountId = window.trevyroSessionService.getUser()._id
      ...
      $(window).off('$trevyro:ready.messages')
      $(window).on('$trevyro:ready.messages', function () {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success) {
              ...
              $.event.trigger('$trevyro:chat:conversation:deleted', {
      ...
      // On user Typing
      $(window).on('$trevyro:chat:typing.conversation', function (event, data) {
      ...
      $(window).on('$trevyro:chat:stoptyping.conversation', function (event, data) {
        const convoListItem = $('#convo-list').find('li[data-conversation-id="' + data.cid + '"]')
        if (convoListItem.length > 0) {
      ...
      // On Chat Message
      $(window).on('$trevyro:chat:message.conversation', function (event, data) {
        const message = {
          _id: data.messageId,
        ...
        if (convoListItem.length > 0) {
          ...
          if (convoUL.length > 0) {
            ...
            // const $injector = angular.injector(['ng', 'trevyro'])
            // $injector.invoke([
            //   '$compile',
            //   '$rootScope',
